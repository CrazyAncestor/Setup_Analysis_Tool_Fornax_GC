...
def SetICs(Halo, Stellar, GC, r_orbit,r_cutoff):
    ...
    print("GC Vx :"+str(-v*np.sin(theta) *(halo_cutoff_mass)/(halo_cutoff_mass+GC_mass)))
    print("GC Vy :"+str( v*np.cos(theta) *(halo_cutoff_mass)/(halo_cutoff_mass+GC_mass)))
    print("Halo Vx :"+str(v*np.sin(theta) *(GC_mass)/(halo_cutoff_mass+GC_mass)))
    print("Halo Vy :"+str(-v*np.cos(theta) *(GC_mass)/(halo_cutoff_mass+GC_mass)))

    r = np.logspace(np.log10(halo_rs/100),np.log10(r_cutoff),1000)
    dens = np.array([density(rho_halo,halo_rs,r[i],halo_type)+density(rho_stellar,stellar_rs,r[i],stellar_type) for i in range(len(r))])
    create_table('profile_Fornax.txt',r,dens)

    r = np.logspace(np.log10(GC_rs/10),np.log10(GC_rt),1000)
    dens = np.array([density(rho_GC,GC_rs,r[i],GC_type) for i in range(len(r))])
...
if __name__== '__main__':
    ...
    if Paper == 'Cole et al. 2012':
        ...
        SetICs(Halo, Stellar, GC, r_orbit, r_cutoff)

    elif Paper == 'Boldrini et al. 2019':
        r_orbit = 0.4
        r_cutoff = 6.
